...
export interface ProdutoItemProps {
  produto: Product;
  produtoSelecionado?: (produto: Product) => void;
}

export default function ProdutoItem(props: ProdutoItemProps) {
  const { adicionarItem } = useCarrinho();
  const parcelamento = useParcelamento(props.produto.promotionalPrice);
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Image
          source={{ uri: props.produto.image }}
          style={styles.imagem}
          accessibilityLabel="Imagem do Produto"
        />
        <View style={{ flex: 1 }}>
          <View style={styles.reviewContainer}>
            <NotaReview nota={props.produto.note} />
          </View>
          <Text style={styles.nome}>{props.produto.name}</Text>
          <Text style={styles.precoCheio}>
            de {Coin.formate(props.produto.basePrice)}
          </Text>
          <View style={styles.precoContainer}>
            <Text style={{ color: "white" }}>por</Text>
            <Text style={styles.preco}>
              {Coin.formate(props.produto.promotionalPrice)}
            </Text>
          </View>
          <Text style={styles.parcelamento}>
            ou {parcelamento.installmentQuantity}x de{" "}
            {Coin.formate(parcelamento.installmentValue)}
          </Text>
        </View>
      </Pressable>
      <Pressable
