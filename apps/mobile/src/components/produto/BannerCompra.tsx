...
export default function BannerCompra(props: BannerCompraProps) {
  ...
  const parcelamento = useParcelamento(produto.promotionalPrice);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.containerPreco}>
        ...
        >
          <Text style={styles.valorCheio}>de R$ {produto?.basePrice}</Text>
          <View style={styles.precoPromocional}>
            <Text style={styles.valor}>por</Text>
            <Text style={styles.valorDestaque}>
              {Coin.formate(produto?.promotionalPrice)}
            </Text>
          </View>
        </View>
        <View style={{ flex: 1, alignItems: "center" }}>
          <Text style={styles.valor}>
            até {parcelamento.installmentQuantity}x de
          </Text>
          <Text style={styles.valor}>
